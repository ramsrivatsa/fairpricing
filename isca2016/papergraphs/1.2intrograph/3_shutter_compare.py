...
fList = [s.strip() for s in open('singlevm_23/shutter_cpi_1000.txt').readlines()]
fList2 = [s.strip() for s in open('2_degradation/%s.txt' % filename).readlines()]
fList3 = [s.strip() for s in open('3_degradation/%s.txt' % filename).readlines()]
fList4 = [s.strip() for s in open('4_degradation/%s.txt' % filename).readlines()]
fList5 = [s.strip() for s in open('2_prior/%s.txt' % filename2).readlines()]
fList6 = [s.strip() for s in open('3_prior/%s.txt' % filename2).readlines()]
fList7 = [s.strip() for s in open('4_prior/%s.txt' % filename2).readlines()]

executionground = 'execution_time_1000'
executionprior = 'execution_time_4'
fList8 = [s.strip() for s in open('2_degradation/%s.txt' % executionground).readlines()]  #degradation
fList9 = [s.strip() for s in open('2_prior/%s.txt' % executionprior).readlines()]  #degradation
fList10 = [s.strip() for s in open('3_degradation/%s.txt' % executionground).readlines()]  #degradation
fList11 = [s.strip() for s in open('3_prior/%s.txt' % executionprior).readlines()]  #degradation
fList12 = [s.strip() for s in open('4_degradation/%s.txt' % executionground).readlines()]  #degradation
fList13 = [s.strip() for s in open('4_prior/%s.txt' % executionprior).readlines()]  #degradation
...
plt.subplot(2, 1, 1)
plt.bar( x, shutter_vm2, width, color="r", label='Application + 1 corunners (libquantum)' )
#plt.text(0.5, 0.5, ha='center', va='center', size=24, alpha=.5)
plt.bar( x+width, shutter_vm3, width, color="b", label='Application + 2 corunners (libquantum) ' )
plt.bar( x+width+width, shutter_vm4, width, color="g", label='Application + 3 corunners (libquantum)')
plt.ylim(0,60)
plt.ylabel( 'Error' )
