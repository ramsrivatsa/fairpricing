...
for a in xrange(0,len(runningList),2):
    ...
    for (b,a) in enumerate(phaseList_single[i]):
        if a[0][0] == '-':
            ...
            instruction_list2.append(float(a[1])*1.26)
    ...
    ax.plot(instruction_list2,cpi_list2, color='g',  label= 'mcf w/o mcf')
...
mars_awesome_plt.savefig('mcfsnapshotmcf.eps', bbox_inches='tight');
mars_awesome_os.popen('epstopdf mcfsnapshotmcf.eps');
